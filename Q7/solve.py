...
def test_query(q):
    ...
    if set(results) == set(targets):
        ...
        false_pos = set(results) - set(targets)
        false_neg = set(targets) - set(results)
        print("Query:", q)
        print("  FP:", len(false_pos), false_pos)
        print("  FN:", len(false_neg), false_neg)

test_query(["site:civicdata.example", "-site:legacy.civicdata.example", "-site:mirror.civicdata-cdn.example", "filetype:pdf", "intitle:audit", "-inurl:drafts", "after:2019", "before:2026", '"evaluation notes"'])
